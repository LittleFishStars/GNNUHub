//! 一次性抓取教务系统前端静态资源（交互式验证码版）
//!
//! 与 `js_recon.rs` 的区别：本示例通过**文件桥**处理验证码——脚本把验证码
//! 图片写到 `tools/js-recon/captcha.png`，然后等待 `tools/js-recon/captcha.txt`
//! 出现并读取其中的字符。这样整个抓取在**一个进程内**完成，会话不会断，
//! 无需拆成两次运行。
//!
